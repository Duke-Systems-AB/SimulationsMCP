...
def test_resolve_db_indices_escapes_every_name(monkeypatch):
    ...
    be = _load_backend()
    fake = _FakeApp(answers=["1", "1", "1"])
    be._resolve_db_indices(fake, 'd" ; Evil("', 't" ; Evil("', 'f" ; Evil("')
    for cmd in fake.executed:
        assert "Evil" not in split_modl(cmd)[0], cmd   # code outside the ModL string literals must not contain the injected call
...
def test_db_set_value_pokes_the_documented_address_shape(monkeypatch):
    """Address is DB:#db:tbl:rec:fld:rec:fld - the record/field pair repeats, and the
    record is ExtendSim's 1-based index: API record 9 is ExtendSim record 10."""
    be = _load_backend()
    ...
    assert address == "DB:#1:2:10:3:10:3", address
    assert value == "42"
...
def test_db_get_records_treats_end_record_as_exclusive(monkeypatch):
    be = _load_backend()
    # resolve(db, tbl) -> 1,1 | DBRecordsGetNum -> 10 | DBFieldsGetNum -> 1
    # | DBFieldGetProperties -> 8192 (DB_FIELDTYPE_REAL_GENERAL) | then one read per record
    fake = _use(monkeypatch, be, _FakeApp(
        answers=["1", "1", "10", "1", "8192"] + ["1.0"] * 10))
    ...
    assert len(reads) == 3
    # API records 0,1,2 are ExtendSim records 1,2,3; API record 3 (ExtendSim 4) is not read
    assert ", 4);" not in "".join(reads), "end_record itself must not be read"
    assert ", 0);" not in "".join(reads), "ExtendSim has no record 0"


def test_db_get_records_clamps_end_record_to_the_table_size(monkeypatch):
    be = _load_backend()
    _use(monkeypatch, be, _FakeApp(
        answers=["1", "1", "2", "1", "8192"] + ["1.0"] * 10))
...
def test_db_delete_records_treats_end_record_as_inclusive(monkeypatch):
    ...
    assert len(delete) == 1
    # API 0..2 (three records) is ExtendSim's 1-based records 1..3
    assert "DBRecordsDelete(1, 1, 1, 3)" in delete[0], delete[0]
...
def test_db_table_info_maps_field_type_codes_to_names(monkeypatch):
    be = _load_backend()
    # resolve -> 1,1 | records=5 | fields=2 | name,type | name,type
    _use(monkeypatch, be, _FakeApp(
        answers=["1", "1", "5", "2", "qty", "4096", "label", "16384"]))
    r = be.db_table_info("db", "t")
    assert r["success"] is True
    assert r["records"] == 5 and r["fieldCount"] == 2
    names = [f["name"] for f in r["fields"]]
    assert names == ["qty", "label"]
    # the live codes of DB_FIELDTYPE_INTEGER_VALUE and DB_FIELDTYPE_STRING_VALUE
    assert [f["type"] for f in r["fields"]] == ["integer", "string"]
